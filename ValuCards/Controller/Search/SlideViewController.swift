...
/*
 ...
 class SlideViewController: UIViewController {
     ...
     private func setupCardItems() {
         let cardItems = imagesAndTitlesAndPrices.map { CardItem(imageName: $0.imageName, title: $0.title, price: $0.price, graphData: []) }
         slideResponseModel = ResponseModel(cardItems: cardItems)
         imageView.isUserInteractionEnabled = true
     }
     ...
     @objc func handlePanGesture(_ gesture: UIPanGestureRecognizer) {
         ...
         case .changed:
             ...
             applyColorFilter()
         case .ended, .cancelled:
             finishPanGesture(gesture)
         default:
             break
         }
     }
     
     private func finishPanGesture(_ gesture: UIPanGestureRecognizer) {
         imageView.alpha = 1.0
         UIView.animate(withDuration: 0.3) {
             self.imageView.transform = .identity
         }
         ...
         if translationX > 0 {
             performSearchCardAction(for: currentTitle)
         } else {
             slideResponseModel.showNextImage()
             updateImageAndTitle()
             resetColorFilter()
     ...
     private func performSearchCardAction(for title: String) {
             CardsModel.shared.searchCards(withName: title) { [weak self] result in
                 switch result {
                 case let .success(card):
                     guard let strongSelf = self else { return }
                     let averagePrice = strongSelf.pricingService.getAveragePrice(from: card.itemSummaries)
     ...
     func navigateToResultViewController(with averagePrice: Double, lowest: Double, highest: Double, currency: String, graphData: [Double]) {
             let storyboard = UIStoryboard(name: "Main", bundle: nil)
             if let resultViewController = storyboard.instantiateViewController(withIdentifier: "Result") as? ResultViewController {
                 resultViewController.averagePrice = averagePrice
                 resultViewController.lowestPrice = lowest
                 ...
                 resultViewController.image = imageView.image
                 resultViewController.cardTitle = slideResponseModel.getCurrentTitle()
                 resultViewController.graphData = graphData
                 navigationController?.pushViewController(resultViewController, animated: true)
             }
         }
     ...
     @IBAction func validateImageButton(_ sender: UIButton) {
         ...
         CardsModel.shared.searchCards(withName: currentTitle) { [weak self] result in
             ...
             case let .success(card):
                 ...
                 let highestPrice = strongSelf.pricingService.getHighestPrice(from: card.itemSummaries)
                 if let currency = card.itemSummaries.first?.price.currency {
                     self?.navigateToResultViewController(with: averagePrice, lowest: lowestPrice, highest: highestPrice, currency: currency, graphData: [])
                 }
             case .failure(_):
                 self?.showAlert(for: .rechercheCarte)
             }
